...
def _get_target_input_embeddings(target_model):
    ...
    if hasattr(target_model, "model") and hasattr(target_model.model, "language_model"):
        return target_model.model.language_model.embed_tokens
    if hasattr(target_model, "language_model"):
        return target_model.language_model.embed_tokens
    if hasattr(target_model, "model") and hasattr(target_model.model, "embed_tokens"):
        return target_model.model.embed_tokens
    assert False, "Target model must expose input embeddings."


def _get_target_output_embeddings(target_model):
    ...
    assert False, "Target model must expose output embeddings/lm_head."
...
class BaseTrainer:
    ...
    def build_models(self):
        ...
        target_embed_tokens = _get_target_input_embeddings(target_model)
        target_lm_head = _get_target_output_embeddings(target_model)
