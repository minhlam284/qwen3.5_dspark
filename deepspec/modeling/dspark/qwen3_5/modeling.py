...
def apply_dspark_rotary_pos_emb(q, k, cos, sin, q_len: int, unsqueeze_dim=1):
    q_cos = cos[..., -q_len:, :].unsqueeze(unsqueeze_dim)
    q_sin = sin[..., -q_len:, :].unsqueeze(unsqueeze_dim)
    k_cos = cos.unsqueeze(unsqueeze_dim)
    k_sin = sin.unsqueeze(unsqueeze_dim)
...
class Qwen3_5DSparkAttention(nn.Module):
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        q, gate = torch.chunk(
            self.q_proj(hidden_states).view(
                bsz, q_len, self.num_attention_heads, self.head_dim * 2
            ),
            2,
            dim=-1,
        )
        ...
        q, k = apply_dspark_rotary_pos_emb(q, k, cos, sin, q_len=q_len)
...
class Qwen3_5DSparkDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3_5DSparkAttention(config=config, layer_idx=layer_idx)
...
class Qwen3_5DSparkModel(Qwen3_5PreTrainedModel):
    _no_split_modules = ["Qwen3_5DSparkDecoderLayer"]
    ...
    def __init__(self, config) -> None:
        ...
        self.layers = nn.ModuleList(
            [
                Qwen3_5DSparkDecoderLayer(config, layer_idx)
        ...
        self.hidden_norm = Qwen3_5RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
__all__ = [
    "Qwen3_5DSparkModel",
    "Qwen3_5DSparkAttention",
    "Qwen3_5DSparkDecoderLayer",
]
